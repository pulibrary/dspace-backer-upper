RSpec.describe Dspace::Downloader do
  ...
  let(:item_fixture_path) { root_path.join("spec", "fixtures", "dspace", "1672.json") }
  ...
  before do
    ...
    stub_request(:get, %r{https://dataspace.princeton.edu/rest/bitstreams/.*/retrieve})
      .to_return(status: 200, body: File.open(root_path.join("spec", "fixtures", "files", "sample.pdf")), headers: {})
  ...
  context "when a resource has items" do
    it "skips any items it fails on, emptying the directory" do
      ...
      allow(DspaceBackup.logger).to receive(:info)
      ...
      expect(DspaceBackup.logger).to have_received(:info).with(/Failed to download 88435\/dsp01tx31qh74p/)
  ...
  describe "#download_item" do
    ...
    context "when there are no bitstreams" do
      ...
      it "logs a debug message" do
        ...
        expect(DspaceBackup.logger).to have_received(:info).with(/No bitstreams for /)
