module Dspace
  class Downloader
    ...
    def initialize(collection_handle:, dspace_token:, collection_dir: nil, collection_resource: nil, ark_mapping: nil, progress: nil, logger: DspaceBackup.logger, order_items: false)
    ...
    def write_metadata(item, item_path)
      mms_id = self.mms_id(item)
      File.write(item_path.join("figgy_metadata.json"), {title: item.title, source_metadata_identifier: mms_id, identifier: item.ark, local_identifier: [item.handle, item.id.to_s].select(&:present?)}.compact.to_json)
    end
    ...
    def download_bitstream(item, item_path, bitstream)
      ...
      File.binwrite(bitstream_path, resp.body)
    end

    def download_path
      DspaceBackup.download_path
    end
